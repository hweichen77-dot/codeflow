...
const DEFAULT_GLOW_COLOR = '94, 210, 156';
...
export function MagicBentoCard({
  ...
  return (
    <div ref={cardRef} onClick={onClick} className={cn('mb-card', span, className)}>
      {children}
    </div>
  );
}

function GlobalSpotlight({ gridRef, disableAnimations = false, spotlightRadius = DEFAULT_SPOTLIGHT_RADIUS, glowColor = DEFAULT_GLOW_COLOR }) {
...
export function MagicBentoGrid({ children, className, disableAnimations = false }) {
  ...
  return (
    <>
      ...
      <div
        ref={gridRef}
        className={cn('mx-auto grid max-w-6xl grid-cols-1 gap-4 md:auto-rows-[16rem] md:grid-cols-6', className)}
      >
        {children}
      </div>
    </>
